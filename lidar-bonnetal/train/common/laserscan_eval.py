...
class LaserScan1:
  ...
  def open_scan(self, filename,efilename):
    ...
    eclass = np.fromfile(efilename, dtype=np.int32)
    self.eclass = eclass.reshape((-1))

    # 해당 class 포인트 삭제
    self.del_class = [10,40,48,50,70]
...
class SemLaserScan1(LaserScan1):
  ...
  def __init__(self,  sem_color_dict=None, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, max_classes=300):
    super(SemLaserScan1, self).__init__(project, H, W, fov_up, fov_down)
    self.reset()
  ...
  def reset(self):
    """ Reset scan members. """
    super(SemLaserScan1, self).reset()
