...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=str,
      required=False,
      default="/mnt/kkm/result/Rangenet21_fs/data_cfg.yaml",
  ...
  for i in np.arange(len(seq_label_names)):
      for j in np.arange(len(seq_pred_names)):
          if seq_label_names[i].split('/')[-1] == seq_pred_names[j].split('/')[-1]:
              scan_names.append(seq_scan_names[i])
              label_names.append(seq_label_names[i])
              pred_names.append(seq_pred_names[j])
