...
from tasks.semantic.modules.segmentator_sq import *
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(
            ...
            if self.gpu:
                ...
                proj_labels_5 = F.upsample(proj_labels, size=(h, w // 8), mode='nearest').squeeze(1).cuda(
                    non_blocking=True).long()
                proj_labels_4 = F.upsample(proj_labels, size=(h, w // 8), mode='nearest').squeeze(1).cuda(
                    non_blocking=True).long()
                proj_labels_3 = F.upsample(proj_labels, size=(h, w // 4), mode='nearest').squeeze(1).cuda(
                    non_blocking=True).long()
                proj_labels_2 = F.upsample(proj_labels, size=(h, w // 2), mode='nearest').squeeze(1).cuda(
                    non_blocking=True).long()
