...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.state:
            output = F.upsample(output, (384,216), None, 'bilinear')
...
class ENet(nn.Module):
    def __init__(self, only_encode=False):
        ...
        self.state = only_encode
        self.encoder = Encoder(3, only_encode=only_encode)
    ...
    def save_checkpoint(self, logdir, suffix=""):
        ...
        # torch.save(self.state.state_dict(), logdir +
        #            "/state_dict" + suffix)
        torch.save(self.encoder.state_dict(), logdir +
                   "/encoder" + suffix)
