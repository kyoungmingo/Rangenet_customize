...
from tasks.semantic.modules.ENet import *
...
class Trainer():
  def __init__(self, ARCH, DATA, datadir, logdir, path=None):
    ...
    # tensorboard를 활용
    #commen.logger.py 에서 tensorboard활용을 위한 것들이 지정되어 있음.
    #self.writer = SummaryWriter('runs/ENet_training')
    #self.writer = SummaryWriter('runs/RangeNet_training')
    ...
    # logdir/tb
    # info에 학습에 관한 정보를 저장하기 위에 holder 지정
    ...
    for x_cl, w in enumerate(self.loss_w):  # ignore the ones necessary to ignore
      if DATA["learning_ignore"][x_cl]:
        # don't weigh
        # ignore class에 weight를 0을 줌으로써 아예 학습을 하지 않는다.
        self.loss_w[x_cl] = 0
    print("Loss weights from content: ", self.loss_w.data)

    # concatenate the encoder and the head
    # path < pretrained // ARCH 여러 input // get_n_classes class개수
    with torch.no_grad():
     # self.model = Segmentator(self.ARCH,
     #                          self.parser.get_n_classes(),
     #                          self.path)
      self.model = ENet(self.parser.get_n_classes())
    ...
    # optimizer
    # optimizer 매개변수들 ex) lr
    # xxxx_lr 에 parameters()들을 저장
    # CRF와 같이 train 여부에 따라 저장할지 말지 결정하기 위함인듯
  ...
  # save_to_log parameter들을 저장
  # info의 tag train_update 등 value 값들
  @staticmethod
  def save_to_log(logdir, logger, info, epoch, w_summary=False, model=None, img_summary=False, imgs=[]):
    ...
    # scalar랑 hist tensorboard활용을 위해 지정
  ...
  def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10, show_scans=False):
    ...
    for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(train_loader):
      ...
      # for kk, losss in enumerate(loss):
      #   self.running_loss += losss.item()
      #
      # if i % 10 == 9:    # 매 1000 미니배치마다...
      #
      #   # ...학습 중 손실(running loss)을 기록하고
      #   self.writer.add_scalar('training loss',
      #                       self.running_loss / 10,
      ...
      self.running_acc += accuracy.item()
      self.running_iou += jaccard.item()
      # if i % 10 == 9:  # 매 1000 미니배치마다...
        ...
        #                            self.running_acc / 10,
        #                            epoch * len(train_loader) + i)
        # self.writer.add_scalar('training iou',
        #                            self.running_iou / 10,
        #                            epoch * len(train_loader) + i)
