...
from common.laserscan_eval import SemLaserScan1
from common.laserscan_ENet import SemLaserScan

# possible splits
splits = ["train", "valid", "test"]

if __name__ == '__main__':
  parser = argparse.ArgumentParser("./evaluate_iou_2stage.py")
  parser.add_argument(
      '--dataset', '-d',
      type=str,
      required=True,
  ...
  for sequence in test_sequences:
    ...
    label_paths = os.path.join(FLAGS.dataset, "sequences",
                               str(sequence), "labels")
    class_paths = os.path.join("/mnt/han/lidar-bonnetal/train/tasks/semantic/DB_1/predcitions_stage1/sequences/", str(sequence),
                              "predictions")
  ...
  for scan_file, label_file, pred_file, class_file in zip(scan_names, label_names, pred_names, class_names):
    print("evaluating label ", label_file, "with", pred_file)
    # open label
    label = SemLaserScan(project=False)
    label.open_scan(scan_file,class_file)
    label.open_label(label_file,class_file)
    u_label_sem = remap_lut[label.sem_label]  # remap to xentropy format
    if FLAGS.limit is not None:
      u_label_sem = u_label_sem[:FLAGS.limit]

    # open prediction
    pred = SemLaserScan1(project=False)
    pred.open_scan(scan_file,class_file)
